...
export function registerInventoryRoutes(app: Application) {
  ...
  app.get('/api/inventory/costumes', jwtMiddleware, (req: Request, res: Response) => {
    ...
    const inventoryRecords = db
      .get('inventory')
      .filter((record: any) => record.item_type === 'COSTUME' && record.is_active !== false)
      .value()
    ...
    for (const costume of db.get('costumes').value()) {
      costumeById.set(costume.id, costume)
    }

    for (const condition of db.get('inventory_item_conditions').value()) {
  ...
  app.post('/api/inventory/import', jwtMiddleware, (req: Request, res: Response) => {
    ...
    const condition = db.get('inventory_item_conditions').find({ id: conditionId }).value()
    if (!condition) {
    ...
    const baseSku = [prefix, initials, normalizedColorToken].join('-').toUpperCase()
    const existingSkus = db
      .get('inventory')
      .filter((record: any) => typeof record.sku === 'string' && record.sku.startsWith(`${baseSku}-`))
      .map((record: any) => String(record.sku))
      .value()

    const startSeq = getNextSkuSeq(baseSku, existingSkus)
