...
// * GET /employees
app.get('/employees', authMiddleware, (req: Request, res: Response) => {
  const result = queryCollection('employees', req.query, res)
  return res.status(200).json(result)
})

// * GET /employees/:id
app.get('/employees/:id', authMiddleware, (req: Request, res: Response) => {
...
// * POST /employees
app.post('/employees/create', authMiddleware, (req: Request, res: Response) => {
  const {
    full_name,
    phone,
    email,
  ...
  return res
    .status(201)
    .json(db.get('employees').find({ id: created.id }).value())
})
...
app.delete(
  ...
  (req: Request, res: Response) => {
    ...
    if (
      req.query.permanantly &&
      JSON.parse(req.query.permanantly as string | undefined)
    ) {
      db.get('employees').remove({ id }).write()
      // Unlink user if linked
      db.get('users')
        .find({ employee_id: id })
        .assign({ employee_id: null })
        .write()
    } else {
      db.get('employees')
        .find({ id })
